...
#include "Assets/MyRenderPipeline/ShaderLibrary/Deprecated.hlsl"
...
int _WithNoShadow;
Light GetMainLight(float2 screenUV)
{
    Light light = GetMainLight();
    if(_WithNoShadow)
        return light;
    light.shadowAttenuation = MainLightRealtimeScreenSpaceShadow(screenUV);
    return light;
}
